...
function ProjectDetails() {
  ...
  const refreshWorkspace = async () => {
    ...
    await Promise.all([fetchProject(), fetchGenerations()]);
  ...
  const removeGeneration = (generationId) => {
    setGenerations((prev) => prev.filter((item) => item._id !== generationId));
  };
  ...
  const topics = generations.filter((item) => item.type === "topic");

  const questions = generations.filter((item) => item.type === "question");

  const objectives = generations.filter((item) => item.type === "objective");

  const literature = generations.filter((item) => item.type === "literature");

  const methodology = generations.filter((item) => item.type === "methodology");

  const abstracts = generations.filter((item) => item.type === "abstract");
  ...
  return (
    <div className="p-6">
      ...
      <div className="bg-white rounded-2xl shadow p-8 mb-8">
        ...
        {project?.selectedTopic && (
          <div className="mt-6 bg-green-50 border border-green-300 rounded-xl p-5">
            ...
            <p className="text-lg">{project.selectedTopic}</p>
          </div>
        )}

        {generations.length > 0 && (
          <button
            onClick={() => exportProjectPDF(project, generations)}
      ...
      <div className="grid md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        <StatCard title="Topics" value={topics.length} color="text-blue-600" />
      ...
      <AIWorkspace project={project} onGenerate={refreshWorkspace} />
      ...
      {!loading && generations.length === 0 && (
        <div className="bg-white rounded-2xl shadow p-10 text-center">
          ...
          <h2 className="text-2xl font-bold mt-4">Nothing Generated Yet</h2>
