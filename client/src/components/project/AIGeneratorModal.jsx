...
function AIGeneratorModal({ type, project, onClose, onGenerate }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const body = {
        type,
        course,
        prompt: type === "topic" ? interest : project?.selectedTopic,

        projectId: project?._id,
      };

      const response = await authFetch(`${endpoints[type]}`, {
        method: "POST",
        headers: {
      ...
      if (!data.success) {
        ...
        if (data.message.includes("You have reached your monthly limit")) {
      ...
      const saveResponse = await authFetch("/api/generations", {
        ...
        body: JSON.stringify({
          projectId: project?._id,
          type,
          input: type === "topic" ? interest : project?.selectedTopic,
          output: result,
        }),
      });
    ...
    <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
      <div className="bg-white w-full max-w-xl rounded-2xl p-8 shadow-xl">
        ...
        {type === "topic" ? (
          ...
          <>
            <label className="font-medium">Research Topic</label>

            <div className="mt-2 bg-green-50 border border-green-300 rounded-lg p-4">
              <p className="font-semibold">{project?.selectedTopic}</p>
        ...
        {loading && (
          <div className="mt-6 bg-blue-50 border border-blue-200 rounded-xl p-5 flex items-center gap-4">
            ...
            <div>
              <p className="font-semibold text-blue-700">ResearchHub AI</p>
